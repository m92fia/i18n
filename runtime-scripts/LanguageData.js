...
module.exports = {
  ...
  updateSceneRenderers () { // very costly iterations
    ...
    // walk all nodes with localize image and update
    let allLocalizedSprites = []
    for (let i = 0; i < rootNodes.length; ++i) {
      let sprites = rootNodes[i].getComponentsInChildren('localizedImage')
      Array.prototype.push.apply(allLocalizedSprites, sprites)
    }
    console.log('---- allLocalizedSprites.length', allLocalizedSprites.length)
    for (let i = 0; i < allLocalizedSprites.length; ++i) {
      let sprite = allLocalizedSprites[i]
      sprite.updateSprite()
    }
